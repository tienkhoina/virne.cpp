...
struct ExactModelData {
    ...
    std::vector<double> node_capacity_scales;
    std::vector<double> link_demands;
    std::vector<double> link_capacities;
    std::vector<double> link_capacity_scales;
    ...
    std::vector<std::uint8_t> physical_edge_is_bridge;
...
ExactModelData prepare_model(
    ...
    data.node_capacity_scales.assign(data.node_resource_count, 0.0);
    for (std::size_t p = 0U; p < data.physical_node_count; ++p) {
        for (std::size_t r = 0U;
             r < data.node_resource_count;
             ++r) {
            data.node_capacity_scales[r] = std::max(
                data.node_capacity_scales[r],
                data.node_capacity(p, r));
        }
    }
    ...
    data.physical_edge_is_bridge.assign(data.physical_edge_count, 0U);
    // A bridge is a direct topology-fragmentation signal: consuming its
    // residual capacity has no alternate physical edge to preserve. This is
    // computed once at the cold model boundary and becomes a fixed coefficient
    // in the objective; no graph traversal reaches an OR-Tools hot loop.
    std::vector<std::vector<std::pair<Vertex, std::size_t>>> bridge_adjacency(
        data.physical_node_count);
    for (std::size_t edge = 0U;
         edge < data.physical_edge_count;
         ++edge) {
        const auto& item = data.physical_edges[edge];
        if (item.source == item.target) {
            continue;
        }
        bridge_adjacency[item.source].emplace_back(item.target, edge);
        bridge_adjacency[item.target].emplace_back(item.source, edge);
    }
    std::vector<std::int64_t> discovery(
        data.physical_node_count, -1);
    std::vector<std::int64_t> low(
        data.physical_node_count, -1);
    std::int64_t discovery_clock = 0;
    const auto visit_bridge = [&]
        (const auto& self, const Vertex vertex, const std::size_t parent_edge)
        -> void {
        discovery[vertex] = discovery_clock;
        low[vertex] = discovery_clock++;
        for (const auto [neighbor, edge] : bridge_adjacency[vertex]) {
            if (edge == parent_edge) {
                continue;
            }
            if (discovery[neighbor] < 0) {
                ...
                if (low[neighbor] > discovery[vertex]) {
                    data.physical_edge_is_bridge[edge] = 1U;
                }
            } else {
    ...
    for (Vertex vertex = 0U;
         vertex < data.physical_node_count;
         ++vertex) {
        if (discovery[vertex] < 0) {
            visit_bridge(visit_bridge, vertex, std::numeric_limits<std::size_t>::max());
        }
    }
    ...
    data.link_capacity_scales.assign(data.link_resource_count, 0.0);
    for (std::size_t e = 0U; e < data.physical_edge_count; ++e) {
        for (std::size_t r = 0U;
             r < data.link_resource_count;
             ++r) {
            data.link_capacity_scales[r] = std::max(
                data.link_capacity_scales[r],
                data.link_capacity(e, r));
        }
    }
...
SolvedModel solve_model(
    ...
    const ExactRiskParameters& risk_parameters) {
    ...
    SolverBackend backend = create_solver();
    ...
    for (std::size_t e = 0U; e < virtual_edges; ++e) {
        const bool path_integral = data.has_integral_link_flow(e);
        for (std::size_t edge = 0U;
             edge < data.physical_edge_count;
             ++edge) {
    ...
    const auto validate_weight = [](const double value, const char* name) {
        if (!std::isfinite(value) || value < 0.0) {
            throw std::invalid_argument(
                std::string("exact_with_risk invalid ") + name +
                " weight");
        }
        return value;
    };
    const double scarcity_weight = validate_weight(
        risk_parameters.scarcity_weight, "scarcity");
    const double balance_weight = validate_weight(
        risk_parameters.balance_weight, "balance");
    const double criticality_weight = validate_weight(
        risk_parameters.criticality_weight, "criticality");

    const auto residual_skew = [](const double sum,
                                  const double square_sum,
                                  const std::size_t count) {
        if (count == 0U) {
            return 0.0;
        }
        return square_sum - sum * sum / static_cast<double>(count);
    };

    // This is a linear marginal surrogate for the Resource Fragmentation
    // Degree (RFD) idea: residual resource vectors that become more skewed,
    // or consume a scarce lane, receive a larger coefficient. The full RFD
    // neighborhood formula is nonlinear; this fixed-state delta is the part
    // that can be added without changing the exact feasibility rows.
    const auto compute_node_risk = [&](const std::size_t v,
                                       const std::size_t p) {
        if (data.node_resource_count == 0U) {
            return 0.0;
        }
        double before_sum = 0.0;
        double before_square_sum = 0.0;
        double after_sum = 0.0;
        double after_square_sum = 0.0;
        double scarcity = 0.0;
        for (std::size_t r = 0U;
             r < data.node_resource_count;
             ++r) {
            const double capacity = data.node_capacity(p, r);
            const double scale = std::max(
                data.node_capacity_scales[r], kEpsilon);
            const double demand = data.node_demand(v, r);
            const double before_value = capacity / scale;
            const double after_value =
                std::max(0.0, capacity - demand) / scale;
            before_sum += before_value;
            before_square_sum += before_value * before_value;
            after_sum += after_value;
            after_square_sum += after_value * after_value;
            scarcity += demand / (capacity + kEpsilon);
        }
        const double lane_count = static_cast<double>(
            data.node_resource_count);
        return scarcity_weight * scarcity / lane_count +
            balance_weight * (
                residual_skew(after_sum, after_square_sum,
                              data.node_resource_count) -
                residual_skew(before_sum, before_square_sum,
                              data.node_resource_count));
    };
    const auto compute_link_risk = [&](const std::size_t e,
        ...
        for (std::size_t r = 0U;
             r < data.link_resource_count;
             ++r) {
            const double capacity = data.link_capacity(edge, r);
            const double scale = std::max(
                data.link_capacity_scales[r], kEpsilon);
            const double unit = data.link_resource_ratio(e, r);
            const double before_value = capacity / scale;
            const double after_value =
                std::max(0.0, capacity - unit) / scale;
            before_sum += before_value;
            before_square_sum += before_value * before_value;
            after_sum += after_value;
            after_square_sum += after_value * after_value;
            scarcity += unit / (capacity + kEpsilon);
        }
        const double lane_count = static_cast<double>(
            data.link_resource_count);
        const double mean_utilization = scarcity / lane_count;
        const double bridge_risk = data.physical_edge_is_bridge[edge] != 0U
            ? mean_utilization
            : 0.0;
        return scarcity_weight * mean_utilization +
            balance_weight * (
                residual_skew(after_sum, after_square_sum,
                              data.link_resource_count) -
                residual_skew(before_sum, before_square_sum,
                              data.link_resource_count)) +
            criticality_weight * bridge_risk;
    };

    // Freeze each marginal coefficient once. The physical-edge coefficient is
    // shared by its two directed arcs, so the objective hot loop performs only
    // dense numeric loads and OR-Tools coefficient writes.
    std::vector<double> node_risks(virtual_nodes * physical_nodes, 0.0);
    for (std::size_t v = 0U; v < virtual_nodes; ++v) {
        for (std::size_t p = 0U; p < physical_nodes; ++p) {
            const std::size_t index = placement_index(v, p);
            if (placement[index]->ub() <= 0.0) {
                continue;
            }
            node_risks[index] = compute_node_risk(v, p);
            if (!std::isfinite(node_risks[index])) {
                solution.description =
                    "exact_with_risk node coefficient is not finite";
                return SolvedModel{std::move(solution), false};
            }
        }
    }
    const auto link_risk_index = [physical_edge_count =
                                      data.physical_edge_count](
                                     const std::size_t e,
                                     const std::size_t edge) noexcept {
        return e * physical_edge_count + edge;
    ...
    for (std::size_t e = 0U; e < virtual_edges; ++e) {
        for (std::size_t edge = 0U;
             edge < data.physical_edge_count;
             ++edge) {
            const std::size_t index = link_risk_index(e, edge);
            if (path_upper_bounds[path_upper_index(e, edge)] <= 0.0) {
                continue;
            }
            link_risks[index] = compute_link_risk(e, edge);
            if (!std::isfinite(link_risks[index])) {
                solution.description =
                    "exact_with_risk link coefficient is not finite";
                return SolvedModel{std::move(solution), false};
            }
        }
    }

    // Normalize the secondary potential so its complete feasible-domain
    // contribution is strictly below one. A one-unit route-length reduction
    // therefore dominates every possible fragmentation improvement, without
    // introducing a huge big-M coefficient into SCIP.
    long double risk_upper_bound = 0.0L;
    for (std::size_t v = 0U; v < virtual_nodes; ++v) {
        for (std::size_t p = 0U; p < physical_nodes; ++p) {
            if (placement[placement_index(v, p)]->ub() > 0.0) {
                risk_upper_bound += static_cast<long double>(
                    std::fabs(node_risks[placement_index(v, p)]));
            }
        }
    }
    for (std::size_t e = 0U; e < virtual_edges; ++e) {
        for (std::size_t arc = 0U; arc < physical_arcs; ++arc) {
            const std::size_t edge = data.physical_arcs[arc].edge_index;
            const double upper = path_use[path_index(e, arc)]->ub();
            risk_upper_bound += static_cast<long double>(upper) *
                static_cast<long double>(
                    std::fabs(link_risks[link_risk_index(e, edge)]));
        }
    }
    if (!std::isfinite(risk_upper_bound)) {
        solution.description =
            "exact_with_risk normalization bound is not finite";
        return SolvedModel{std::move(solution), false};
    }
    long double risk_coefficient_bound = 0.0L;
    for (const double value : node_risks) {
        risk_coefficient_bound = std::max(
            risk_coefficient_bound,
            static_cast<long double>(std::fabs(value)));
    }
    for (const double value : link_risks) {
        risk_coefficient_bound = std::max(
            risk_coefficient_bound,
            static_cast<long double>(std::fabs(value)));
    }
    const long double risk_denominator =
        1.0L + risk_upper_bound + risk_coefficient_bound;
    const auto normalize_risk = [risk_denominator](const double value) {
        return static_cast<double>(
            static_cast<long double>(value) / risk_denominator);
    };
    for (double& value : node_risks) {
        value = normalize_risk(value);
    }
    for (double& value : link_risks) {
        value = normalize_risk(value);
    }

    auto* objective = solver->MutableObjective();
    const auto set_risk_objective = [&](const bool include_route) {
        objective->Clear();
        ...
        for (std::size_t v = 0U; v < virtual_nodes; ++v) {
            for (std::size_t p = 0U; p < physical_nodes; ++p) {
                objective->SetCoefficient(
                    placement[placement_index(v, p)],
                    node_risks[placement_index(v, p)]);
            }
        }
        for (std::size_t e = 0U; e < virtual_edges; ++e) {
            for (std::size_t arc = 0U; arc < physical_arcs; ++arc) {
                const std::size_t edge = data.physical_arcs[arc].edge_index;
                objective->SetCoefficient(
                    path_use[path_index(e, arc)],
                    (include_route ? 1.0 : 0.0) +
                        link_risks[link_risk_index(e, edge)]);
            }
        }
    };
    const auto solve_to_optimality = [&]() {
        ...
        solution.description =
            ...
            ? "exact_with_risk did not prove objective optimality"
            : "exact_with_risk model has no optimal solution";
        return false;
    };

    const bool has_continuous_path_flow = std::any_of(
        data.link_flow_integral.begin(),
        data.link_flow_integral.end(),
        [](const std::uint8_t value) { return value == 0U; });
    if (!has_continuous_path_flow) {
        // Integral q makes total route flow discrete. The normalized complete
        // risk range is below one, so one route-flow unit is dominant.
        set_risk_objective(true);
        if (!solve_to_optimality()) {
            return SolvedModel{std::move(solution), false};
        }
    } else {
        // Floating-only resource lanes require continuous q. Use two exact
        // phases because no finite scalar can dominate every arbitrarily
        // small continuous primary-objective difference.
        objective->Clear();
        objective->SetMinimization();
        for (std::size_t e = 0U; e < virtual_edges; ++e) {
            for (std::size_t arc = 0U; arc < physical_arcs; ++arc) {
        ...
        const double optimal_route_flow = objective->Value();
        if (!std::isfinite(optimal_route_flow)) {
            solution.description =
                "exact_with_risk primary objective is not finite";
            return SolvedModel{std::move(solution), false};
        }
        MPConstraint* route_lock = solver->MakeRowConstraint(
            optimal_route_flow, optimal_route_flow);
        for (std::size_t e = 0U; e < virtual_edges; ++e) {
            for (std::size_t arc = 0U; arc < physical_arcs; ++arc) {
                route_lock->SetCoefficient(
                    path_use[path_index(e, arc)], 1.0);
            }
        }
        set_risk_objective(false);
        if (!solve_to_optimality()) {
            return SolvedModel{std::move(solution), false};
        }
    }
...
ExactWithRiskSolver::ExactWithRiskSolver(
    ...
    : Solver(std::move(dependencies), std::move(config)),
      ...
      risk_parameters_(risk_parameters),
      random_(&random) {}

const ExactSolverParameters& ExactWithRiskSolver::parameters() const noexcept {
    return parameters_;
}

const ExactRiskParameters& ExactWithRiskSolver::risk_parameters() const noexcept {
    return risk_parameters_;
}

core::Solution ExactWithRiskSolver::solve(const SolverInstance& instance) {
    return solve_model(
...
MutableSolverResult ExactWithRiskSolver::solve_mutable(
...
SolverId register_exact_with_risk_solver(
    ...
    return registry.register_solver(
        ...
        [parameters, risk_parameters, random_ptr](
            SolverDependencies dependencies,
            SolverConfig config) -> std::unique_ptr<Solver> {
            return std::make_unique<ExactWithRiskSolver>(
