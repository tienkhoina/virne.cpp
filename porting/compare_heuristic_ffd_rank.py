#!/usr/bin/env python3
"""Compact AST-isolated differential for heuristic FFDRankSolver."""
...
class FakeFFDNodeRank:
    def __call__(self, network):
        ranked = zip(network.nodes, network.node_resources)
        return dict(sorted(ranked, key=lambda item: item[1], reverse=True))
...
def load_target(source: pathlib.Path):
    ...
    selected = [
        ...
        if node.name in {"BaseNodeRankSolver", "FFDRankSolver"}
    ]
    FakeSolverRegistry._registry = {}
    namespace: dict[str, Any] = {
        "__name__": "heuristic_ffd_rank_isolated",
        "Controller": object,
        "Recorder": object,
        "Counter": object,
        "Logger": object,
        ...
        "FFDNodeRank": FakeFFDNodeRank,
    }
    isolated = ast.fix_missing_locations(
        ast.Module(body=selected, type_ignores=[]))
    exec(compile(isolated, str(source), "exec"), namespace)
    registered = FakeSolverRegistry._registry
    if list(registered) != ["ffd_rank"]:
        raise RuntimeError(f"isolated registry drift: {list(registered)}")
    ffd_solver = registered["ffd_rank"]
    if ffd_solver.type != "node_ranking":
        raise RuntimeError(f"isolated category drift: {ffd_solver.type}")
    return ffd_solver
...
def cases():
    return [
        (
            "ffd_permutation",
            virtual_network(3, [], [2, 9, 5], []),
            physical_network(3, [], [6, 3, 12], []),
            False,
        ),
        (
            "stable_tie",
            virtual_network(3, [], [8, 7, 1], []),
            physical_network(3, [], [10, 3, 10], []),
            False,
        ),
        (
            "link_success",
            virtual_network(2, [(0, 1)], [3, 2], [1]),
            physical_network(3, [(0, 1), (1, 2)], [10, 10, 10], [10, 10]),
            False,
        ),
        (
            "partial_node_failure",
            virtual_network(2, [], [9, 8], []),
            physical_network(2, [], [10, 7], []),
            False,
        ),
        (
            "disconnected_route_failure",
            virtual_network(2, [(0, 1)], [2, 1], [1]),
...
def python_payload(ffd_solver, workers):
    results = []
...
def main() -> int:
    ...
    ffd_solver = load_target(args.source)
    for workers in WORKERS:
        expected = python_payload(ffd_solver, workers)
    ...
    result = {
        ...
        "boundaries": {
            ...
            "python_isolation": (
                "Only exact BaseNodeRankSolver and FFDRankSolver AST nodes "
